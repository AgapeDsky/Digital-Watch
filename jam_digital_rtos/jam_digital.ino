...
uint32_t stopwatchCensecs = 0; 

/* Timer time params */
uint32_t timerSecs = 0;
uint32_t timerMins = 0;

/* Stopwatch time params */
...
void setup() {
  ...
  /*Initialize thread*/
  xTaskCreate(displayCallback, "Task A", 50, NULL, tskIDLE_PRIORITY + 4, NULL);
  ...
  xTaskCreate(stopwatchCallback, "Task E", 128, NULL, tskIDLE_PRIORITY + 2, NULL);
  xTaskCreate(alarmCallback, "Task D", 64, NULL, tskIDLE_PRIORITY + 1, NULL);
}
...
void displayCallback(void* pvParameters) {
  ...
  while(1) {
    switch (displayMode) {
      ...
      case 4 : displayTime(stopwatchSecs, stopwatchCensecs); break;
      case 5 : displayTime(stopwatchMins, stopwatchSecs); break;
    }
    switch (setupMode) {
      /**
  ...
  while(1) {
    if(displayMode == 4) {
      if(setupMode == false){            
        ...
        bool attachedOnce2 = false;
        
        while(stopwatchstart) {
          if (displayMode == 4 || displayMode == 5) {
            if (attachedOnce2 == 0) {
              attachInterrupt(digitalPinToInterrupt(buttonInc), stopStopwatch, RISING);
              attachedOnce2 = 1;
            }
            else {
              // do nothing
          ...
          timeMillis = micros();
          while((micros()-timeMillis)<8700){};
          addStopwatchcensecs();
        }
        detachInterrupt(digitalPinToInterrupt(buttonInc));
        timeMillis = millis();
        while((millis()-timeMillis)<500){};
        while(!resetstopwatch){
          if(digitalRead(buttonNext)){
            resetStopwatch();
          }
        }
...
void addStopwatchcensecs(){
  if(stopwatchCensecs < 100){
    stopwatchCensecs++;
  }
  else{
    stopwatchCensecs=0;
